...
class TestPosPayInvoice(common.TransactionCase):
    def setUp(self):
        super(TestPosPayInvoice, self).setUp()
    ...
    def test_process(self):
        ...
        in_invoice = self.env['pos.invoice.in'].with_context(
        ...
        in_invoice.run()
        out_invoice = self.env['pos.invoice.out'].with_context(
            active_ids=session.ids, active_model='pos.session'
        ).create({
            'invoice_id': self.invoice_out.id,
            'amount': 50.0
        ...
        session.action_pos_session_closing_control()
        self.assertEqual(self.invoice_out.residual, 50.)
